...
class TCGCard extends HTMLElement {
    ...
    template.innerHTML = `
      <style>
        ...
        .tcg-display {
          ...
          transition: scale ${EXPAND_TRANSITION_TIME}ms ease-out, translate ${EXPAND_TRANSITION_TIME}ms ease-out, rotate 250ms ease-in;
        }
  ...
  /**
   * Gère le début d'un toucher sur la carte
   */
  handleTouchStart(e) {
  ...
  /**
   * Gère le déplacement d'un toucher sur la carte
   */
  handleTouchMove(e) {
    e.preventDefault();
    const touch = e.touches[0];
  ...
  /**
   * Gère la fin d'un toucher sur la carte
   */
  handleTouchEnd() {
    this.endInteraction();
  }
  ...
  /**
   * Gère le clic sur la carte (agrandissement/réduction)
   */
  handleClick() {
    if (this.expanded) {
      this.close();
  ...
  /**
   * Démarre l'interaction avec la carte
   */
  ...
  /**
   * Met à jour la transformation de la carte en fonction de la position du pointeur
   */
  ...
  /**
   * Termine l'interaction avec la carte
   */
  endInteraction() {
  ...
  /**
   * Centre la carte au milieu de l'écran pour l'agrandissement
   */
  ...
  resetCardPosition(skipTransition = false) {
    const scale =
      this.proxy.getBoundingClientRect().height / this.card.clientHeight || 0.5;
    if (skipTransition) {
      this.card.style.setProperty("transition", "none");
    }
  ...
  /**
   * Gère les clics sur le document pour fermer la carte
   */
  ...
  /**
   * Gère les appuis de touches pour fermer la carte
   */
